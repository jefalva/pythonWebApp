...
if v == "true":
	...
	if "balsubmit1" in data and t == "true":
		...
		for row in c.execute('SELECT * FROM accounts WHERE accountnum = %s AND name = %s', (accountnum,name)):
			print("<b>Account Number: </b>" + row[0] + "<br>")
			print("<b>Customer Name: </b>" + row[1] + "<br>")
			print("<b>Previous Balance: </b>" + str(row[2]) + "<br>")
		c.execute('UPDATE accounts SET balance = balance + %s WHERE accountnum = %s AND name = %s', (addmoney,accountnum,name)):
		conn.commit()
		for row in c.execute('SELECT * FROM accounts WHERE accountnum = %s AND name = %s', (accountnum,name)):
	...
	#withdraw money
		...
		for row in c.execute('SELECT * FROM accounts WHERE accountnum = %s AND name = %s', (accountnum,name)):
			...
			print("<b>Customer Name: </b>" + row[1] + "<br>")
			print("<b>Previous Balance: </b>" + str(row[2]) + "<br>")
		c.execute('UPDATE accounts SET balance = balance - %s WHERE accountnum = %s AND name = %s', (submoney,accountnum,name)):
		conn.commit()
		for row in c.execute('SELECT * FROM accounts WHERE accountnum = %s AND name = %s', (accountnum,name)):
			print("<b>New Balance: </b>" + str(row[2]) + "<br>")

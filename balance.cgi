...
if v == "true":
	...
	if "balsubmit1" in data and t == "true":
		...
		c.execute('UPDATE accounts SET balance = balance + '+addmoney+' WHERE accountnum=(?) AND name=(?)', (accountnum,name)):
		conn.commit()
		for row in c.execute('SELECT * FROM accounts WHERE accountnum=(?) AND name=(?)', (accountnum,name)):
	...
	#withdraw money
		...
		for row in c.execute('SELECT * FROM accounts WHERE accountnum=(?) AND name=(?)', (accountnum,name)):
			...
			print("<b>Customer Name: </b>" + row[1] + "<br>")
			print("<b>Previous Balance: </b>" + str(row[2]) + "<br>")
		c.execute('UPDATE accounts SET balance = balance - '+submoney+' WHERE accountnum=(?) AND name=(?)', (accountnum,name)):
		conn.commit()
		for row in c.execute('SELECT * FROM accounts WHERE accountnum=(?) AND name=(?)', (accountnum,name)):
			print("<b>New Balance: </b>" + str(row[2]) + "<br>")
